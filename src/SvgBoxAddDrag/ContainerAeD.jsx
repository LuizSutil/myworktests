...
export const ContainerAed = () => {
    ...
    const [itemList, setItemList] = useState({
        ...
        1:{title:'RemoteControl', controllingKey:'None', selected: false, exists:true}
    })
    ...
    const addToList = () =>{
        var _x = optionsList
        var _y = _x.filter(item => item != "None")
        console.log(_y)
    }
    ...
    const addItem = (top,left) =>{
        ...
        const _x = itemList
        _x[_entry] = {title:'PressureBlower', state:false, selected:false, exists:true}
        setItemList(_x)
        const _z = items
        ...
        <div>
            ...
            <div className='row'>
                ...
                <div className='canvasColumn'>
                    <text style={{fontSize:23, marginBottom:5}}>Factory Building Canvas Demo</text>
                    <div>
                        {/* Main Canvas */}
                        <SvgCanvas items={items} 
                ...
                {   
                menuState ? <img src={WolferLogo} ></img>  :
                Object.keys(itemList).map((key)=>{
                if (itemList[key].selected){
                    return (<>
                    <text className='subhead'>
                        menu</text>
                    <text style={{marginTop:20, fontWeight:'bold'}}>{itemList[key].title}</text>
                    {itemList[key].controllingKey?  <text style={{marginTop:"20px", marginBottom:'10px'}}>
                            Control Selector</text> : null }
                    {itemList[key].controllingKey? <Dropdown options={optionsList} onChange={(e)=> _onSelect(e.value)} 
                        value={defaultOption} placeholder="Select an option" /> : null }       
                    </>)
                    }
                })                               
                }
