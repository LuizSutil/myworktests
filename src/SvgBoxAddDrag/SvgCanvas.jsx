...
export const SvgCanvas = ({items, setItemsFun, itemList, addSgv}) => {
    ...
    const [{isActive},drop] = useDrop(()=>({
        ...
        collect: (monitor) => (
          ...
          isActive: monitor.canDrop() && monitor.isOver() && (monitor.getItemType() === 'svg'),
        }),
    ...
    return (
        ...
        <div ref={drop} style={{...styles, backgroundColor: isActive?  'grey' : '#D9DBDB' }}>
        {/* maps items from  itemlist and returns items inside canvas*/}
        {Object.keys(items).map((key)=> {
            const {left, top} = items[key];
            const {title, state, controllingKey, selected, exists} = itemList[key];
            if(exists===true) {
           return( <SvgBox 
                selected = {selected}
                title={title} 
